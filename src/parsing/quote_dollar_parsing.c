...
static int	ft_replace_dollar_to_var(char *str, char **env)
{
	...
	i = 0;
	size_of_var = 0;
	while (str[size_of_var] != ' ' && str[size_of_var] != '\"' && str[size_of_var] != '\0' && (ft_isalpha(str[size_of_var]) || ft_isdigit(str[size_of_var])) && str[size_of_var] != '$' )
		size_of_var++;
	while (env[i])
	{
...
int	ft_process_check_quote(char *str, char **env, char type_of_quote, char **end_double)
{
    ...
    {
		ft_memmove(&str[0], &str[1], ft_strlen(str));
       while ((*str && *str != '\"' && *str != '$' && type_of_quote == '\"')
				|| (*str && *str != '\'' && type_of_quote == '\''))
            str++;
...
void	verif_is_dollar(char **str, int *return_var, char **env, char **end_single)
{
	...
	else if (!ft_strncmp(*str, "$?", 2) && (!*end_single || *end_single <= *str))
		ft_pipe_return_err(*str);
	*return_var = 1;
}
